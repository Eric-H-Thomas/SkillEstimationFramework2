...
DEFAULT_MIN_PLATE_X = -2.13
DEFAULT_MAX_PLATE_X = 2.13
DEFAULT_MIN_PLATE_Z = -2.50
DEFAULT_MAX_PLATE_Z = 6.60
...
def baseball_execution_skill_key(execution_skill: float, rho: float = DEFAULT_EXECUTION_RHO) -> str:
    ...
    return "|".join(map(str, [execution_skill, execution_skill])) + f"|{rho}"
...
def build_log_lambda_grid(
    *,
    ...
    """Canonical log-lambda grid aligned with ``JointMethodQRE`` for baseball."""

    raw = np.logspace(math.log10(lambda_min), 3.6, num=num_lambda_grid, dtype=float)
    if num_lambda_grid >= 2:
        raw[-1] = min(raw[-1], lambda_max)
    return np.log(raw)
...
def load_processed_statcast() -> pd.DataFrame:
    ...
    if not PROCESSED_PICKLE.is_file():
        raise FileNotFoundError(
            ...
            "Run Environments/Baseball/downloadStatcastCSVs.py and dataTake2.manageData first."
        )
...
def _load_processed_bundle() -> tuple[
    ...
    with PROCESSED_PICKLE.open("rb") as handle:
        loaded = pickle.load(handle)
    _, batter_indices, model_targets_plate_x, model_targets_plate_z, possible_targets_feet, possible_targets_for_model = (
        loaded[0]
    )
    return (
        np.asarray(batter_indices),
        np.asarray(model_targets_plate_x, dtype=float),
        np.asarray(model_targets_plate_z, dtype=float),
        np.asarray(possible_targets_feet, dtype=float),
        np.asarray(possible_targets_for_model, dtype=float),
    )


def build_strike_zone_grids(delta: float = DEFAULT_DELTA) -> tuple[StrikeZoneGrids, np.ndarray]:
    """Build strike-zone grids from processed Statcast metadata."""
    ...
    grids = StrikeZoneGrids(
        ...
        model_targets_plate_x=model_targets_plate_x,
        model_targets_plate_z=model_targets_plate_z,
        possible_targets_feet=possible_targets_feet,
        possible_targets_for_model=possible_targets_for_model,
    )
    return grids, batter_indices


def _load_model(batter_indices: np.ndarray) -> tuple[nn.Module, torch.device]:
...
def _utility_grid_from_row(
    ...
    ypred = modelTake2.prediction_func(model, torch_batch_x, torch_batch_y)
...
def _build_pdfs_and_covs(
    ...
    for execution_skill in execution_skills:
        ...
        cov = domain.getCovMatrix([skill, skill], DEFAULT_EXECUTION_RHO)
        all_covs[key] = cov
    ...
    return pdfs_per_execution_skill, all_covs
...
def build_baseball_runtime(
    ...
    *,
    ...
    model, device = _load_model(batter_indices)
    pdfs_per_execution_skill, all_covs = _build_pdfs_and_covs(rng, grids, execution_skills)
...
def build_pitch_observation(
    ...
    for execution_skill in execution_skills:
        ...
        evs_per_execution_skill[key] = np.asarray(evs, dtype=float)
...
def get_agent_pitch_rows(
    ...
    *,
    ...
    agent_data = all_data.query(f"pitcher == {pitcher_id} and pitch_type == '{pitch_type}'")
    agent_data = agent_data.sort_values(by=["game_date"], ascending=False)
