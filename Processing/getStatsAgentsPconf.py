...
if __name__ == "__main__":
	...
	prdFile = args.resultsFolder + os.sep + "ProcessedResultsFiles" + os.sep + "resultsDictInfo"
	...
	pconfPerXskill = pconf(rng,args.resultsFolder,args.domain,domainModule,spacesModule,args.mode,args,wrap)
	...
	for a in processedRFsAgentNames:
		...
		prat = getPercentRationalGivenParams(pconfPerXskill,minMaxX,x,p)
		...
		if aType in infoPerType:
			...
			infoPerType[aType]["percents"].append(prat)
		else:
			infoPerType[aType] = {"xs":[x],"ps":[p],"percents":[prat]}

		# if "X4.8856" in a:
		#	code.interact(f"{a}...", local=dict(globals(), **locals()))
		...
		for at in infoPerType:
			...
			if each == outfileV:
				...
				for eachName in infoPerType[at]["names"]:
					if info[eachName]['numExps'] > 1:
						print(f"\t\t{eachName} | x {info[eachName]['numExps']}",file=each)
					else:
						print(f"\t\t{eachName}",file=each)
			...
			for b in buckets:
				print(f"\t\tTotal # of agents with <= {b} % rationality: {len(infoPerBuckets[at][b])}")
