...
mainFolderName = scriptPath.split(f"Processing{os.sep}makePlotsForPaperBaseballMulti.py")[0]
...
def getBoardPlots(pitcherID,pitchType,saveAt,cmapStr):
	...
	for x in allInfo:
		...
		covMatrix = sys.modules["domain"].getCovMatrix(x[:-1],x[-1])
		pdfsPerXskill[str(x)] = sys.modules["domain"].getNormalDistribution(rng,covMatrix,delta,spaces.targetsPlateXFeet,spaces.targetsPlateZFeet)

		fig,ax = plt.subplots()
		cs = ax.scatter(spaces.possibleTargetsFeet[:,0],spaces.possibleTargetsFeet[:,1],c = pdfsPerXskill[str(x)])
	...
	tempArgs = [5,1]
	agentData = spaces.getAgentData("chunks",pitcherID,pitchType,tempArgs)
	...
	for row in agentData.itertuples():
		...
		for x in allInfo:
			...
			EVs = convolve2d(Zs,pdfsPerXskill[str(x)],mode="same",fillvalue=minUtility)
			...
			try:
				...
				plt.savefig(f"{saveAt}{fileName}-xskill{x}.jpg",bbox_inches="tight")
