...
# FROM: https://github.com/kartikay-bagla/bump-plot-python/blob/master/bumpplot.py
def bumpchart(df, show_rank_axis= True, rank_axis_distance= 1.1, 
	...
	left_yaxis.set_xticklabels(left_yaxis.get_xticklabels(), rotation = 90)

	return axes


def makeBumpPlot(resultsDict,methodsLists,resultsFolder,givenBeta,labels):

	makeFolder(resultsFolder,"bumpPlots")
	...
	for pitcherID in resultsDict.keys():
		...
		for pitchType in resultsDict[pitcherID]:
			...
			with open(f"{rdFile}-pitcherID{pitcherID}-pitchType{pitchType}",'rb') as handle:
				loadedDictInfo = pickle.load(handle)


			for method in methodsLists[0]:

				if method not in info[pitcherID]:
					info[pitcherID][method] = {"estimatesPerPitchType":[], "avgEstimate":0.0}

				est = None

				if "BM" in method:  
					tempM, beta, tt = getInfoBM(method)
					if loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)] != {}:						
						est = loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)][-1]
				elif "BM" not in method:
					if loadedDictInfo[pitcherID][pitchType]["estimates"][method] != {}:
						est = loadedDictInfo[pitcherID][pitchType]["estimates"][method][-1]
						
				if est != None:
					numObs = loadedDictInfo[pitcherID][pitchType]["numObservations"]
					info[pitcherID][method]["estimatesPerPitchType"].append(est)
	...
	for pitcherID in info:
		for m in methodsLists[0]:
			info[pitcherID][m]["avgEstimate"] = np.sum(info[pitcherID][m]["estimatesPerPitchType"])/len(info[pitcherID][m]["estimatesPerPitchType"])



	allPitchers = list(info.keys())
	...
	for m in methodsLists[0]:
		...
		for pitcherID in allPitchers:
			estimates.append([pitcherID,info[pitcherID][m]["avgEstimate"]])

		# Sort estimates
		estimates = np.array(estimates)
		estimates = np.sort(estimates)

		# Map back to pitchers
		ranks[m]["rankedPitchers"] = estimates[:,1]
		ranks[m]["estimates"] = estimates[:,0]


	for i in range(len(labels)):

		methods = methodsLists[i]
		label = labels[i]
		...
		for pitcherID in info:
			...
			for m in methods:
				# Find picher rank in this current method
				# Plus 1 cause index starts at 0
				rank = np.where(ranks[m]["rankedPitchers"]==pitcherID)[0][0]+1
				data[pitcherID].append(rank)
		...
		plt.tight_layout()
		plt.savefig(f"{resultsFolder}{os.sep}plots{os.sep}bumpPlots{os.sep}plot{label}.png")
	...
	for ma in methodsAllProbs:
		...
		for pitcherID in resultsDict.keys():
			...
			for pitchType in resultsDict[pitcherID]:
				...
				if loadedDictInfo[pitcherID][pitchType]["allProbs"] == {}:
					continue

				if loadedDictInfo[pitcherID][pitchType]["allProbs"][ma] == {}:
					continue
				...
				if "NJT" in ma:
					for e in range(len(loadedDictInfo[pitcherID][pitchType]["allProbs"][ma][-1])):
						loadedDictInfo[pitcherID][pitchType]["allProbs"][ma][-1][e] = loadedDictInfo[pitcherID][pitchType]["allProbs"][ma][-1][e][0]

			
				lastProbs = np.array(loadedDictInfo[pitcherID][pitchType]["allProbs"][ma][-1])
				...
				elif "BM" not in actualMethod:
					if "-xSkills" not in actualMethod:
				...
				if "BM" in ma: 
					...
					if loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)] != {}:						
						estimate1 = loadedDictInfo[pitcherID][pitchType]["estimates"][tt]["BM-MAP"][str(beta)][-1]
						estimate2 = loadedDictInfo[pitcherID][pitchType]["estimates"][tt]["BM-EES"][str(beta)][-1]
				elif "BM" not in ma:
					if "EES" not in actualMethod and "MAP" not in actualMethod:
						if "EES" not in m2:
					...
					if loadedDictInfo[pitcherID][pitchType]["estimates"][toCheck] != {}:
						estimate1 = loadedDictInfo[pitcherID][pitchType]["estimates"][m1][-1]
						estimate2 = loadedDictInfo[pitcherID][pitchType]["estimates"][m2][-1]
					else:
						estimate1 = None
						estimate2 = None
...
def compareToGivenMethod(resultsDict,methods,resultsFolder,compareTo="OR",thresObservations=50,thresSame=0.10):
		...
		if "BM" in method or "xSkill" in method:
			...
			for pitcherID in resultsDict.keys():
				...
				for pitchType in resultsDict[pitcherID]:
					...
					if loadedDictInfo[pitcherID][pitchType]["numObservations"] > thresObservations:
						...
						if est != None:
							...
							estOR = loadedDictInfo[pitcherID][pitchType]["estimates"][keyCompareTo][-1]
							thres = estOR*0.10
	...
	with open(f"Experiments{os.sep}baseball{os.sep}info.pkl",'rb') as handle:
		...
		for pitchType in resultsDict[pitcherID]:
			...
			for ma in methodsAllProbs:
				...
				if "BM" in ma or "NJT" in ma:
					...
					lastProbs = loadedDictInfo[pitcherID][pitchType]["allProbs"][ma][-1]
	...
	for pitcherID in resultsDict.keys():
		...
		for pitchType in resultsDict[pitcherID]:
			...
			for method in methods:
				...
				if "pSkills" not in method: 
					...
					if "BM" in method:  
						tempM, beta, tt = getInfoBM(method)
						if loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)] != {}:
							plt.semilogx(range(1,loadedDictInfo[pitcherID][pitchType]["numObservations"]+1),loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)], lw='2.0', label = method)
							makePlot = True
					elif "BM" not in method and loadedDictInfo[pitcherID][pitchType]["estimates"][method] != {}:
						plt.semilogx(range(1,loadedDictInfo[pitcherID][pitchType]["numObservations"]+1),loadedDictInfo[pitcherID][pitchType]["estimates"][method], lw='2.0', label = method)
						numObs = len(loadedDictInfo[pitcherID][pitchType]["estimates"][method])
						makePlot = True
			...
			if makePlot:
				...
				fig.suptitle(f"Pitcher: {pitcherNames[pitcherID]} ({pitcherID}) | Pitch Type: {pitchType} | Num Observations: {loadedDictInfo[pitcherID][pitchType]['numObservations']}")
			...
			for method in methods:
				...
				if "pSkills" in method:      
					plt.semilogx(range(loadedDictInfo[pitcherID][pitchType]["numObservations"]),loadedDictInfo[pitcherID][pitchType]["estimates"][method], lw='2.0', label = method)
					makePlot = True
	...
	for pitcherID in resultsDict.keys():
		...
		for pitchType in resultsDict[pitcherID]:
				...
				try:
					...
					if "pSkills" not in method:
						...
						if "BM" in method:  
							tempM, beta, tt = getInfoBM(method)
							if loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)] != {}:
								plt.semilogx(range(1,loadedDictInfo[pitcherID][pitchType]["numObservations"]+1),loadedDictInfo[pitcherID][pitchType]["estimates"][tt][tempM][str(beta)],c=color, lw='2.0', label = tempM + "-" + str(beta))
								makePlot = True
						elif "BM" not in method and loadedDictInfo[pitcherID][pitchType]["estimates"][method] != {}:
							plt.semilogx(range(1,loadedDictInfo[pitcherID][pitchType]["numObservations"]+1),loadedDictInfo[pitcherID][pitchType]["estimates"][method],c=color, lw='2.0', label = method)
							makePlot = True
			...
			if makePlot:
				...
				plt.title(f"Pitcher: {pitcherNames[pitcherID]} ({pitcherID}) | Pitch Type: {pitchType} | Num Observations: {loadedDictInfo[pitcherID][pitchType]['numObservations']}")
	...
	for pitcherID in resultsDict.keys():
		...
		for pitchType in resultsDict[pitcherID]:
			...
			if makePlot:
				...
				plt.margins(0.05)
				plt.title(f"Pitcher: {pitcherNames[pitcherID]} ({pitcherID}) | Pitch Type: {pitchType} | Num Observations: {len(loadedDictInfo[pitcherID][pitchType]['estimates'][method])}")
...
if __name__ == "__main__":
	...
	for pitcherID in resultsDict.keys():
		...
		try:
			...
			with open(f"Experiments{os.sep}baseball{os.sep}pitcherNames.pickle","rb") as infile:
	...
	# Need to check
	makeBumpPlot(resultsDict,methodsLists,args.resultsFolder,givenBeta,labels)
