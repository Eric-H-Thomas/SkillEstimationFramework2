...
def get_scoresPREV(slices,resolution):
    ...
    for i in range(len(X)):
        for j in range(len(Y)):
            action = [X[i],Y[j]]
            # S[i,j] = get_reward_for_action(slices,action)
            S[i,j] = npscore(slices,action[0],action[1])
...
def get_symmetric_normal_distributionPREV(rng,XS,resolution):
...
def getInfoOnBoardOnlyPREV(X,Xs,Ys,EV):
...
def compute_expected_value_curve(rng,slices,X,resolution,returnZn=False):

    # tt = perf_counter()
    XYn,Zn = get_symmetric_normal_distribution(rng,X,resolution)
...
def compute_expected_value_curve_prev(rng,slices, X, resolution, returnZn = False):

    Xn,Yn,Zn = get_symmetric_normal_distribution(rng,X,resolution)

    Xs,Ys,Zs = get_scores(slices, resolution)
    ...
    # Return only info about targets & EVs that are actually within the board
    onBoardXs,onBoardYs,onBoardEVs = getInfoOnBoardOnly(X,Xs,Ys,EVs)
    ...
    # Returns pdfs as well, used on script for testing
    if returnZn:
        return Xs,Ys,EVs,onBoardEVs,Zn
    # Returns targets & EVs
    else:
        return Xs,Ys,EVs,onBoardEVs
...
def get_optimal_action_and_value(rng,S,X,resolution,returnZn=False): 
    ''' Get the target for a given xskill level and resolution '''   

    if returnZn:
        Xn, Yn, EV, onBoardEVs, Zn = compute_expected_value_curve(rng,S,X,resolution,returnZn)
    else:
        Xn, Yn, EV, onBoardEVs = compute_expected_value_curve(rng,S,X,resolution,returnZn)
...
def get_expected_values_and_optimal_action(rng,S,X,resolution,returnZn=False): 
    ''' Get all the targets for a given xskill level and resolution
        as well as the one with the max EV '''

    if returnZn:
        XY, EV, onBoardEVs, Zn = compute_expected_value_curve(rng,S,X,resolution,returnZn)
    else:
        XY, EV, onBoardEVs = compute_expected_value_curve(rng,S,X,resolution,returnZn)
...
def get_expected_values_and_optimal_action_prev(rng,S,X,resolution,returnZn=False):
    ''' Get all the targets for a given xskill level and resolution
        as well as the one with the max EV '''
    ...
    # all_ts = {"all_ts_x": Xn, "all_ts_y": Yn}
    
    # Return target that will give max ev and the actual max ev
    # as well as all the other targets and evs (all the information from the convolution)
    if returnZn:
        return EV,ts,EV[mxi][myi],onBoardEVs,Zn
    else:
        return EV,ts,EV[mxi][myi],onBoardEVs
...
def sample_noisy_action(rng,S,X,a,noiseModel=None):

    # If noise model was not given, proceed to get it
    if noiseModel == None:
        N = draw_noise_sample(rng,X**2)
...
def simulate_board_hits(numTries):
    ...
    # xSkills = [1.0, 5.0, 26.9, 64.6, 100.0]
    xSkills = np.linspace(0.001,150.0,num=200)
    ...
    for xs in xSkills:
        ...
        for tries in range(int(numTries)):
            ...
            nx, ny = sample_noisy_action(S, xs, action)
            noisyAction = [nx, ny]
...
def drawBoardWithEVsForDiffXskillsAndResolutions():
    ...
    # xSkills = [1.0, 5.0, 26.9, 64.6, 100.0]
    # xSkills = [30.928]
    xSkills = np.linspace(28.5, 45.5, num = 8)
    ...
    for xs in xSkills:
        ...
        for res in resolutions:
            ...
            for s in range(len(S)):
                ...
                # all_ts, EV, xy, ev, Zn = get_expected_values_and_optimal_action(rng,S,xs,res, returnZn = True)
                EV, xy, ev, onBoardEVs = get_expected_values_and_optimal_action(rng,state,xs,res)
                ...
                nx, ny = sample_noisy_action(rng,S, xs,[xy[0],xy[1]])
                ...
                plt.title("Execution Noise Level: " + str(round(xs,4))) # + " | Resolution: " + str(res))
                ax.set_xticks([])
                ax.set_yticks([])
                ...
                #plt.show()
                plt.savefig("xSkill-" + str(int(xs)) + "-resolution" + str(res)+ "-state" + str(s)+".png")
...
if __name__ == "__main__":
    ...
    drawBoardWithEVsForDiffXskillsAndResolutions()
    exit()
    numTries = 10_000

    xSkills, allPercentHits = simulate_board_hits(numTries)
    ...
    # To filter for each percent (step = 1)
    for eachPercent in range(100,-1,-1):

        info[eachPercent] = {"xs":[],"percents":[],
                            # "minP":None,"maxP":None,
                            "minX":None,"maxX":None,
                            "widthX":None}

        for i in range(len(allPercentHits)):

            if allPercentHits[i] >= eachPercent and allPercentHits[i] < prevPercent:
                info[eachPercent]["percents"].append(allPercentHits[i])
                info[eachPercent]["xs"].append(xSkills[i])

        prevPercent = eachPercent


    for eachPercent in info.keys():

        if info[eachPercent]["percents"] != []:
            # minP = min(info[eachPercent]["percents"])
            # maxP = max(info[eachPercent]["percents"])

            # i_minP = info[eachPercent]["percents"].index(minP)
            # i_maxP = info[eachPercent]["percents"].index(maxP)
            
            minX = min(info[eachPercent]["xs"])
            maxX = max(info[eachPercent]["xs"])


            # info[eachPercent]["minP"] = minP
            # info[eachPercent]["maxP"] = maxP

            info[eachPercent]["minX"] = minX
            info[eachPercent]["maxX"] = maxX

            widthX =  abs(maxX-minX)
            info[eachPercent]["widthX"] = widthX
    ...
    for eachPercent in info.keys():
        if info[eachPercent]["percents"] != []:
            print(f"Percent: {eachPercent} | minX: {round(info[eachPercent]['minX'],4)} | maxX: {round(info[eachPercent]['maxX'],4)} | widthX: {round(info[eachPercent]['widthX'],4)}")
    print("-"*20)
