...
#
# Submit the H-JEEDS high-data-anchor availability sweep as a Slurm array
# Each array task computes one anchor-count scenario, and a final dependent job
# aggregates the scenario folders and zips the output root

set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: submit_hjeeds_anchor_availability_sensitivity.sh [options]

Submit the H-JEEDS high-data-anchor availability ablation to Slurm. By default
this launches 6 scenario tasks:
...
Slurm options:
  --job-name NAME                Scenario array job name (default: hjeeds-anchors).
...
num_seeds="500"
base_seed=""
output_dir="HJEEDS/results/hierarchical_darts_anchor_availability_sensitivity"
python_bin=""

job_name="hjeeds-anchors"
qos="normal"
partition=""
account=""
time_limit="23:00:00"
...
array_script="${script_dir}/run_hjeeds_anchor_availability_sensitivity.sbatch"
...
echo "Submitting ${scenario_count} scenario tasks."
echo "Anchor agents: 0, 1, 2, 5, 10, 25"
