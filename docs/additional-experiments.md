...
- The agents-per-bucket ablation defaults to 5 scenarios: five agents-per-bucket values under the default hyperprior condition, so the study varies only population size.
