...
def onlineExperiment(args,xskill,agents,env,estimatorsObj,subsetEstimators,tag,counter,seedNum,rng,indexOR,rerun=False):
	...
	for agent in agents:
		...
		if Path(f"{statusFile}.txt").is_file() and not args.rerun and (env.domain_name in ["baseball", "baseball-multi"]):
...
def main():
	...
	args.xSkillsGiven = False
	args.pSkillsGiven = False

	# Try to keep even so that dynamic agents can have both 
	# (good to bad) and (bad to good) combinations
	args.numXskillsPerExp = 2
	args.numPskillsPerExp = 1
	args.numRhosPerExp = 1
	...
	args.resultsFolder = f"Experiments-DynamicAgents-NumObservations{args.numObservations}-NumParticles{args.numParticles[0]}"
	

	# UNCOMMENT WHEN TESTING DYNAMIC AGENTS
	args.dynamic = True
	...
	rf = f"Experiments{os.sep}{args.domain}{os.sep}{args.resultsFolder}{os.sep}"

	folders = ["Experiments",f"Experiments{os.sep}{args.domain}",rf,
	...
	for i in range(numExperiments):
		...
		if args.domain in ["baseball","baseball-multi","soccer"]:
			...
			counter = onlineExperiment(args,None,[agent],env,estimators,subsetEstimators,tag,counter,seedNum,rng,indexOR)
